 ...
 * @param cwd
 * @param pkg
 * @returns @yrui/eslint-config/index
 * @returns @yrui/eslint-config/react
 * @returns @yrui/eslint-config/typescript/index
 * @returns @yrui/eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    '@yrui/eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
